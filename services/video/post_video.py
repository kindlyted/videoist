...
# Playwright 视频上传
def xhs_video_upload(video_path, cover_path, title, desc, cookies_data=None, acct_info=None):
    """上传视频到小红书
    ...
    Args:
        ...
        title: 视频标题
        desc: 视频描述
        cookies_data: cookies数据(dict)，优先使用
        acct_info: cookie文件路径（用于向后兼容）
    """
    with sync_playwright() as p:
        ...
        try:
            if cookies_data:
                ...
                context.add_cookies(json.load(open(acct_info)))
            else:
                raise Exception("未提供cookies数据")
        ...
        try:
            ...
            desc_editor.click()
            page.keyboard.type(desc)  # 模拟真实输入
            print("✅ 内容填写完成")
        except Exception as e:
...
def dy_video_upload(video_path, cover_path, title, desc, cookies_data=None, acct_info=None):
    """上传视频到抖音
...
def sph_video_upload(video_path, cover_path, title, desc, cookies_data=None, acct_info=None):
    """上传视频到视频号
