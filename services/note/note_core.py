...
if __name__ == "__main__":
    ...
    load_dotenv()
    file_path = "./storage/input/2.pdf"
    prompt_path = "./static/prompts/keshihua0.prompt"
    timestamp = datetime.now().strftime("%Y%m%d%H%M%S")
    output_path = f"./storage/note_output/{timestamp}.html"  # 使用时间戳作为文件名
    output_pic_path = f"./storage/note_output/{timestamp}.png"
    output_dir = f"./storage/note_output"
    ...
    a, title = process_pdf_to_png(file_path, prompt_path, output_dir)
    print(title,a)
    print(output_pic_path)