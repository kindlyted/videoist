...
@note_bp.route('/notes-generator', methods=['GET', 'POST'])
@jwt_required()
def upload_file():
    ...
    try:
        ...
        # prompt_path = os.path.join(current_app.root_path, 'note', 'static', 'prompts', 'keshihua0.prompt')
        prompt_path = str(Config.PROMPT_DIR / "keshihua0.prompt")
        # output_dir = os.path.join(current_app.root_path, 'storage', 'note_output')
        output_dir = str(Config.STORAGE_FLODER / "note_output")
        output_pic_path = process_pdf_to_png(file_path, prompt_path, output_dir)
        
        return jsonify({
            'success': True,
            'message': '文件处理成功',
            'image_url': url_for('note.static', filename=f'output/{os.path.basename(output_pic_path)}')
        })
