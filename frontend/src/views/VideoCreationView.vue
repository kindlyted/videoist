  ...
  <div class="container mx-auto px-4 py-8">
    ...
    <div class="mb-8 text-center flex flex-col items-center">
      <div class="flex items-center justify-center">
        <h1 class="text-3xl font-bold">{{ $t('videoCreation.title') }}</h1>
        <SparklesIcon class="h-8 w-8 text-blue-500 ml-4" />
      </div>
      <p class="text-gray-600">{{ $t('videoCreation.subtitle') }}</p>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-md p-6 mb-8">
      ...
      <div class="mb-4">
        <label class="block text-gray-700 text-sm font-bold mb-2">
          {{ $t('videoCreation.publishPlatforms') }}
        </label>
        <div class="flex space-x-4">
          <Switch
            id="wordpressSwitch"
            v-model="urlForm.wordpressSwitch"
            :label="$t('videoCreation.publishToWordPress')"
            :disabled="!hasWordPressSites"
          />
          <Switch
            id="wechatSwitch"
            v-model="urlForm.wechatSwitch"
            :label="$t('videoCreation.publishToWeChat')"
        ...
        <div class="flex space-x-4">
          <label class="inline-flex items-center">
            <input
              ...
              class="form-radio"
            />
            <span class="ml-2">{{ $t('videoCreation.recite') }}</span>
          </label>
          <label class="inline-flex items-center">
            <input
              id="modeDialogue"
              ...
              class="form-radio"
            />
            <span class="ml-2">{{ $t('videoCreation.dialogue') }}</span>
          </label>
        </div>
      </div>
      ...
      <div class="flex items-center justify-between">
        ...
        >
          ...
          {{ isProcessing ? $t('videoCreation.processing') : $t('videoCreation.uploadAndProcess') }}
        </button>
      </div>
    ...
    <div class="bg-white rounded-lg shadow-md p-6 mb-8">
      ...
      <div class="mb-4 relative">
        ...
        <textarea
          ...
          class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
          rows="5"
          :placeholder="$t('videoCreation.scriptPlaceholder')"
        ></textarea>
        <div class="absolute bottom-2 right-2 text-gray-500 text-sm">
          {{ $t('videoCreation.characterCount') }}: {{ characterCount }}
        </div>
      </div>
      ...
      <div class="mb-4">
        ...
        >
          ...
          {{ isGeneratingTitle ? $t('videoCreation.generating') : $t('videoCreation.generateTitle') }}
        </button>
      </div>
      ...
      <div class="mb-4">
        ...
        <input
          ...
          class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
          type="text"
          :placeholder="$t('videoCreation.titlePlaceholder')"
        />
      </div>
      ...
      <div class="mb-4">
        <label class="block text-gray-700 text-sm font-bold mb-2" for="cover">
          {{ $t('videoCreation.coverDescription') }}
        </label>
        <input
          id="cover"
          ...
          class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
          type="text"
          :placeholder="$t('videoCreation.coverPlaceholder')"
        />
      </div>
      ...
      <div class="mb-4">
        <label class="block text-gray-700 text-sm font-bold mb-2" for="voice">
          {{ $t('videoCreation.voiceSelection') }}
        </label>
        <select
          id="voice"
          ...
          class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
        >
          <option value="" disabled>{{ $t('videoCreation.selectVoice') }}</option>
          <option 
    ...
    <div class="bg-white rounded-lg shadow-md p-6 mb-8">
      <h2 class="text-xl font-semibold mb-4 flex items-center">
        ...
        {{ $t('videoCreation.videoGeneration') }}
      </h2>
      ...
      <div class="flex space-x-4 mb-4">
        ...
        >
          ...
          {{ isGeneratingVideo ? $t('videoCreation.generating') : $t('videoCreation.generateVideo') }}
        </button>
        <button
          id="uploadVideo"
          ...
          @click="startUploadVideo"
          :disabled="!videoGenerated"
        >
          <SpinnerIcon v-if="isUploadingVideo" />
          <CloudArrowUpIcon v-else class="h-5 w-5 mr-2" />
          <span v-if="isUploadingVideo">{{ $t('videoCreation.uploading') }}</span>
          <span v-else>{{ $t('videoCreation.uploadPlatforms') }}</span>
        </button>
      ...
      <!-- 预览区域 -->
      <div class="mt-4">
        <h3 class="font-semibold mb-2">{{ $t('videoCreation.preview') }}</h3>
        
        <!-- 封面预览和视频预览并列 -->
        <div class="flex flex-wrap gap-4 mb-4">
          <!-- 封面预览 -->
          <div v-if="preview.coverImage" class="flex-1 min-w-[300px]">
            <p class="font-medium mb-2">{{ $t('videoCreation.coverPreview') }}</p>
            <img 
              id="coverImage" 
              :src="preview.coverImage" 
              :alt="$t('videoCreation.coverPreview')" 
          ...
          <div v-if="preview.videoUrl" class="flex-1 min-w-[300px]">
            <p class="font-medium mb-2">{{ $t('videoCreation.videoPreview') }}</p>
            <video 
              id="outputVideo" 
...
const voiceOptions = ref([
  { id: 'voice1', name: t('videoCreation.voice1') },
          { id: 'voice2', name: t('videoCreation.voice2') },
          { id: 'voice3', name: t('videoCreation.voice3') }
])
...
const generateTitle = async () => {
  if (!contentForm.value.article) {
    alert(t('videoCreation.enterScriptContent'))
    return
  }
...
const generateVideo = async () => {
  if (!contentForm.value.article || !contentForm.value.title) {
    alert(t('videoCreation.enterScriptAndTitle'))
    return
  }
...
const showPlatformSelection = ref(false)

// 添加新的状态变量
const showPlatformLoginWindow = ref(false)

// 修改handlePlatformSelect函数
const handlePlatformSelect = async (platformId) => {
  try {
    // 设置当前选中的平台
    currentLoginPlatform.value = platformId

    // 直接调用后端API启动登录流程
    const loginResponse = await api.post('/platform-login', {
      platform: platformId
    })
    ...
    if (loginResponse.data.success) {
      ...
      throw new Error(loginResponse.data.message || '启动登录流程失败')
    }
  } catch (error) {
...
// 修改startUploadVideo函数
const startUploadVideo = async () => {
  if (!videoGenerated.value || !preview.value.videoUrl) {
    alert(t('videoCreation.generateVideoFirst'))
    return
  }
  
  try {
    // 1. 检查平台登录状态
    const statusResponse = await api.get('/platform-login-status')
    const platformStatus = statusResponse.data
    
    // 2. 收集所有未登录的平台
    const allPlatforms = Object.keys(platformStatus)
    const unloggedPlatforms = allPlatforms.filter(platform => !platformStatus[platform])
    
    if (unloggedPlatforms.length > 0) {
      // 如果有未登录的平台，显示登录卡片
      pendingPlatforms.value = unloggedPlatforms
      currentLoginPlatform.value = unloggedPlatforms[0]
...
// 修改uploadVideo函数中处理cookies失效的部分
const uploadVideo = async () => {
  ...
  try {
    const response = await api.post('/publish-video', {
      video_path: preview.value.videoUrl,
      cover_path: preview.value.coverImage,
      title: contentForm.value.title,
      desc: contentForm.value.article
    })

    if (response.status === 200) {
      alert(t('videoCreation.videoUploadedSuccessfully'))
    }
  } catch (error) {
    console.error('上传视频失败:', error)
    if (error.response?.status === 403 && error.response?.data?.invalid_platforms) {
      // 如果是因为cookies失效导致的错误，显示登录卡片
      pendingPlatforms.value = error.response.data.invalid_platforms
...
const onPlatformLogin = async () => {
  const currentPlatform = currentLoginPlatform.value
  showPlatformLoginWindow.value = false
  loginAttempts.value[currentPlatform] = true

  // 移除已登录的平台
  const platformIndex = pendingPlatforms.value.indexOf(currentPlatform)
  if (platformIndex > -1) {
    pendingPlatforms.value.splice(platformIndex, 1)
  }

  // 如果还有其他平台需要登录，继续显示登录窗口
  if (pendingPlatforms.value.length > 0) {
    currentLoginPlatform.value = pendingPlatforms.value[0]
    showLoginDialog.value = true
  } else {
    // 所有平台都已登录，继续上传
...
const fetchVoiceOptions = async () => {
  try {
    ...
    voiceOptions.value = [
      ...
      { id: 'xiaofeng', name: t('videoCreation.xiaofeng') },
      { id: 'xiaomei', name: t('videoCreation.xiaomei') }
    ]
