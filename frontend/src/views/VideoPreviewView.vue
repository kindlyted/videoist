<template>
  <div class="px-container py-container">
    <div class="max-w-4xl mx-auto">
              ...
              <div class="mb-6">
                <div class="relative pb-[56.25%] h-0 rounded-lg overflow-hidden bg-gray-100">
                  ...
                  <img 
                    ...
                    class="absolute inset-0 w-full h-full object-cover"
                  >
                  <div v-if="!video.videoUrl" class="absolute inset-0 flex items-center justify-center">
                    <button 
      ...
      <div v-if="relatedVideos.length > 0" class="mt-6">
...
const fetchVideo = async () => {
  ...
  try {
    const response = await api.get(`/video/${route.params.id}`);
    // 处理后端返回的新格式 { success: true, data: {...} }
    const videoData = response.data.success ? response.data.data : response.data;
    // 转换后端数据格式以匹配前端组件的期望
    video.value = {
      id: videoData.id,
      title: videoData.title,
      description: videoData.description,
      thumbnail: videoData.thumbnail_path || 'https://picsum.photos/seed/default/400/225',
      videoUrl: videoData.file_path, // 添加视频链接
      duration: 0, // 后端没有返回duration字段，暂时设置为0
      createdAt: videoData.created_at,
      status: 'published' // 后端没有返回status字段，暂时设置为published
    };
    relatedVideos.value = [];
  } catch (error) {
