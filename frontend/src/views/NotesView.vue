<template>
  <div class="container mx-auto px-4 py-8">
    <div class="text-center mb-8">
      <h1 class="text-3xl font-bold mb-2">{{ $t('noteCreation.title') }}</h1>
      <p class="text-gray-600">{{ $t('noteCreation.subtitle') }}</p>
    </div>
    ...
    <div class="max-w-2xl mx-auto bg-white rounded-lg shadow-md p-6 mb-8">
      ...
      >
        <div v-if="!selectedFile" class="flex flex-col items-center justify-center">
          ...
          <p class="text-gray-600 mb-2">{{ $t('noteCreation.dragAndDrop') }}</p>
          <p class="text-sm text-gray-500">{{ $t('noteCreation.fileSupport') }}</p>
        </div>
        <div v-else class="flex flex-col items-center justify-center">
          ...
          <p class="text-gray-700 mb-1">{{ $t('noteCreation.selectedFile') }}</p>
      ...
      <div class="mt-6 flex justify-center">
        ...
        >
          ...
          <span v-if="isUploading">{{ $t('noteCreation.processing') }}</span>
          <span v-else>{{ $t('noteCreation.startConversion') }}</span>
        </button>
      </div>
    </div>
    ...
    <div v-if="resultImageUrl" class="max-w-2xl mx-auto bg-white rounded-lg shadow-md p-6">
      <h2 class="text-xl font-semibold mb-4">{{ $t('noteCreation.conversionResults') }}</h2>
...
const handleFileSelect = (event) => {
  ...
  if (file && file.type === 'application/pdf') {
    ...
    errorMessage.value = t('noteCreation.pdfRequired')
  }
}
...
const uploadFile = async () => {
  if (!selectedFile.value) {
    errorMessage.value = t('noteCreation.selectFileFirst')
    return
  }
